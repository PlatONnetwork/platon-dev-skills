> Compatibility note
>
> This file documents upstream Blockscout transaction-related endpoints. It is broader than the current `platon-blockscout` command set, and not every endpoint listed here is guaranteed to be enabled on PlatON mainnet or devnet.
>
> For currently supported skill commands, check `../api-surface.md`. For PlatON-specific behavior and field quirks, check `../endpoint-notes.md`.
...
These are Etherscan-compatible legacy endpoints. In this repository they are reference material only unless a specific `platon-blockscout` command is added for them later.

#### GET /api?module=logs&action=getLogs

Returns event logs filtered by block range, optional contract address, and up to four topic values. Results are capped at 1,000 entries.
